      ...
      Swal.fire({
        ...
        .then((result) => {
          ...
          if (result.isConfirmed) {
            axios.post(`${apibaseurl}whychooseus/change-status`, { _id: salectedids })
              .then((res) => res.data)
              .then((finalres) => {
    ...
    if (salectedids.length >= 1) {
      Swal.fire({
        ...
        .then((result) => {
          ...
          if (result.isConfirmed) {
            axios.post(`${apibaseurl}whychooseus/delete`, { _id: salectedids })
              .then((res) => res.data)
              .then((finalres) => {
                toast.success(finalres._Message)
                WhyChoosedata()
                ...
                if (finalres._Status) {
                  Swal.fire({
                    title: "Deleted!",
                    text: "Your why choose has been deleted.",
                    icon: "success"
                  });
                }
  ...
  return (
    <>
      ...
      <div className=' w-5xl mx-auto min-h-[88vh]'>
        <h2 className='ps-3 py-2'>Home/View_why_choose_us</h2>
        ...
        <div className='shadow p-2 w-5xl mx-auto bg-neutral-100'>
          ...
          <div className='shadow-xs shadow-violet-600 '> <h2 className='shadow-violet-900 text-2xl font-bold px-3 py-1 text-neutral-600'>View_why_choose_us</h2>  </div>
              ...
              <tbody>
                ...
                {data.map((item, i) => (
                  <tr key={i} className="text-center">
                    ...
                    <td className="border p-2"> {item._WhyChooseTital} </td>
                    <td className="border p-2"> {item._WhyChooseOrder} </td>
                    <td className="border p-2"> {item._WhyChooseDiscription}</td>
                    <td className="border py-1">

                      <img className='w-25 h-15 flex justify-self-center object-center' src={path + item._image} alt={item._image} />
                    </td>

                    <td className="border p-2">  {item._WhyChooseStatus ? <button className='bg-green-500 px-2 py-1 rounded-xl text-white font-bold' > Active </button> : <button className='bg-orange-500 px-2 py-1 rounded-xl text-white' > Deactive </button>}      </td>
                    <td className="border p-2"> <button className='bg-red-500 px-2 py-1 rounded-xl text-white font-bold' onClick={() => Deletedata(item._id)} > Delate </button> | <Link to={`/update/whychooseus/${item._id}`} > <button className='bg-blue-500 font-bold px-2 py-1 rounded-xl text-white'> Edit </button></Link>  </td>
                  </tr>
